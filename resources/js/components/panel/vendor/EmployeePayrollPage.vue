            ...
            <div class="col-6 col-md-3" v-for="s in summaryCards" :key="s.label">
                <div class="stat-card">
                    ...
                    <div class="stat-card-body">
                        ...
                        <div class="stat-card-value small">₱{{ formatMoney(s.value) }}</div>
                    </div>
                </div>
            </div>
        </div>
                ...
                <table class="table table-striped table-hover align-middle mb-0">
                    ...
                    <tbody>
                        <tr v-for="p in payrolls" :key="p.id">
                            ...
                            <td class="text-end small">₱{{ formatMoney(p.gross_amount) }}</td>
                            <td class="text-end small text-success">{{ p.bonus > 0 ? "+₱" + formatMoney(p.bonus) : "—" }}</td>
                            <td class="text-end small text-danger">{{ p.manual_deductions > 0 ? "-₱" + formatMoney(p.manual_deductions) : "—" }}</td>
                            <td class="text-end small text-warning">{{ p.cash_advance_deduction > 0 ? "-₱" + formatMoney(p.cash_advance_deduction) : "—" }}</td>
                            <td class="text-end fw-bold small">₱{{ formatMoney(p.net_amount) }}</td>
                            <td class="text-end small text-success">{{ p.total_paid > 0 ? "₱" + formatMoney(p.total_paid) : "—" }}</td>
                            <td class="text-end small" :class="p.remaining_balance > 0 ? 'text-danger' : 'text-success'">
                                {{ p.remaining_balance > 0 ? "₱" + formatMoney(p.remaining_balance) : "✓" }}
                            </td>
                            <td>
                                <span class="m-badge" :class="statusBadge(p.status)">{{ statusLabel(p.status) }}</span>
                            </td>
                            <td>
                                <div class="d-flex gap-1">
                                    ...
                                    <button v-if="['approved', 'partially_paid'].includes(p.status)" class="btn btn-sm btn-outline-primary" title="Add Payout" @click="openPayoutModal(p)">
                                    ...
                                    <button v-if="p.status !== 'paid'" class="btn btn-sm btn-outline-danger" title="Delete" @click="confirmDelete(p)">
            ...
            <div class="d-md-none">
                <div class="member-card" v-for="p in payrolls" :key="'pm' + p.id">
                    <div class="member-card-top">
                        <div>
                            ...
                            <div class="text-muted" style="font-size: 0.75rem">
                                Net: <strong>₱{{ formatMoney(p.net_amount) }}</strong>
                            </div>
                        </div>
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-end">
                                    ...
                                    <li v-if="['approved', 'partially_paid'].includes(p.status)">
                                    ...
                                    <li v-if="p.status !== 'paid'">
                                        <a class="dropdown-item text-danger" href="#" @click.prevent="confirmDelete(p)"><i class="bi bi-trash me-2"></i>Delete</a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    ...
                    <div class="member-card-tags ps-0 mt-1">
                        <span class="small text-muted">Gross: ₱{{ formatMoney(p.gross_amount) }}</span>
                        <span class="small text-danger" v-if="p.remaining_balance > 0"> · Balance: ₱{{ formatMoney(p.remaining_balance) }}</span>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div v-if="modalMode === 'create'" class="mb-3">
                            ...
                            <div v-else-if="suggestion" class="alert py-2 small mb-0" :class="suggestion.daily_rate > 0 ? 'alert-info' : 'alert-warning'">
                                ...
                                <template v-if="suggestion.daily_rate > 0">
                                    &times; ₱{{ formatMoney(suggestion.daily_rate) }} daily rate = <strong>₱{{ formatMoney(suggestion.gross_amount) }}</strong> gross.
                                </template>
                                ...
                                <span v-if="suggestion.suggested_ca > 0"> &nbsp;· Pending CA: ₱{{ formatMoney(suggestion.suggested_ca) }}</span>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                <label class="form-label form-label-sm">Manual Deductions (₱)</label>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" class="form-control" v-model="form.cash_advance_deduction" min="0" step="0.01" />
                                <div class="form-text text-warning" v-if="suggestedCa > 0 && modalMode === 'create'">Pending advances: ₱{{ formatMoney(suggestedCa) }}</div>
                            </div>
                            <div class="col-12">
                            ...
                            <div class="col-12">
                                <div class="p-3 rounded bg-light border d-flex justify-content-between align-items-center">
                                    ...
                                    <span class="fw-bold fs-5">₱{{ formatMoney(netPreview) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body" v-if="payoutTarget">
                        ...
                        <div class="d-flex justify-content-between mb-3 small">
                            ...
                            <span class="fw-bold text-danger">Balance: ₱{{ formatMoney(payoutTarget.remaining_balance) }}</span>
                        </div>
                        <div class="row g-3">
                            <div class="col-md-6">
                        ...
                        <div v-if="payoutTarget.payouts_count > 0" class="mt-3">
                            ...
                            <div v-else>
                                <div v-for="po in existingPayouts" :key="po.id" class="d-flex justify-content-between align-items-center small border-bottom py-1">
                                    <span
                                        >{{ formatDate(po.paid_at) }} · <span class="text-capitalize">{{ po.method }}</span> <span v-if="po.reference_number" class="text-muted">({{ po.reference_number }})</span></span
                                    >
                                    <span class="fw-semibold text-success">₱{{ formatMoney(po.amount) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="modal-dialog modal-sm">
                <div class="modal-content">
                    <div class="modal-header border-0 pb-0">
                        <h5 class="modal-title fw-bold">Delete Payroll?</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body pt-1 text-muted small">This payroll and all related payouts will be permanently deleted.</div>
...
export default {
    ...
    computed: {
        netPreview() {
            ...
            return Math.max(0, gross + bonus - ded - ca);
        },
        summaryCards() {
    ...
    methods: {
        ...
        submitPayroll() {
            ...
            const req = this.modalMode === "create" ? axios.post(`/panel/employees/${this.employee.id}/payrolls`, this.form) : axios.put(`/panel/payrolls/${this.form.id}`, this.form);
        ...
        approvePayroll(p) {
            this.approving = p.id;
            axios
                .post(`/panel/payrolls/${p.id}/approve`)
                .then((res) => {
        ...
        openPayoutModal(p) {
            ...
            if (p.payouts_count > 0) {
                ...
                axios
                    .get(`/panel/payrolls/${p.id}/payouts`)
        ...
        submitPayout() {
            ...
            this.payoutErrors = {};
            axios
                .post(`/panel/payrolls/${this.payoutTarget.id}/payouts`, this.payoutForm)
                .then((res) => {
        ...
        doDelete() {
            ...
            this.deleting = true;
            axios
                .delete(`/panel/payrolls/${this.deleteTarget.id}`)
                .then(() => {
                    this.payrolls = this.payrolls.filter((p) => p.id !== this.deleteTarget.id);
        ...
        statusLabel(s) {
            return { draft: "Draft", approved: "Approved", partially_paid: "Partial", paid: "Paid" }[s] ?? s;
        },
