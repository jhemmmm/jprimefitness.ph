...
|---|---|---|
| `standalone` *(default)* | Single server | Everything in one place. Sync system inert. Pre-existing single-node behavior. |
...
| `RatePlan`, `PTProduct`, `BusinessProfile` | either *(in practice live)* | Admin config; rarely touched offline. |
